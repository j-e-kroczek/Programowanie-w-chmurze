...
export class GameController {
  ...
  @Post('join/:id')
  joinGame(@CognitoUser() user: any, @Param('id') id: string) {
    const game = this.gameService.findOne(id);
    if (!game) {
      throw new HttpException('Game not found', 404);
    }
    if (this.gameService.isGameAvailable(game.id)) {
      const isOk = this.gameService.addPlayerToGame(
        game.id,
        user.nickname,
        user.sub,
      );
  ...
  makeMove(
    ...
    if (!this.gameService.authPlayer(game.id, user.sub)) {
  ...
  @Post(':id/restart')
  @UseInterceptors(ClassSerializerInterceptor)
  restartGame(@CognitoUser() user: any, @Param('id') id: string) {
    const game = this.gameService.findOne(id);
    if (!game) {
      throw new HttpException('Game not found', 404);
    }
  ...
  @Post(':id/quit')
  quitGame(@CognitoUser() user: any, @Param('id') id: string) {
    const game = this.gameService.findOne(id);
    if (!game) {
      throw new HttpException('Game not found', 404);
    }
